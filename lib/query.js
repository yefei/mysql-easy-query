...
class Query {
  ...
  rawQuery(sql, params) {
    return new Promise((resolve, reject) => {
      ...
      this.conn.query(sql, params, (error, results, fields) => {
        debug('query[%o]: %o %o %oms', this.conn.threadId, sql, params, Date.now() - startTime);
  ...
  rawExecute(sql, params) {
    return new Promise((resolve, reject) => {
      const startTime = Date.now();
      this.conn.execute(sql, params, (error, results, fields) => {
        debug('execute[%o]: %o %o %oms', this.conn.threadId, sql, params, Date.now() - startTime);
        if (error) reject(error);
        else resolve(results);
      });
    });
  ...
  async query(arg0, params) {
    if (typeof arg0 === 'string') {
      return this.rawExecute(arg0, params);
    }
    let builder = arg0;
    ...
    if (builder instanceof Builder) {
      ...
      const res = await this.rawExecute({ sql, nestTables: builder.getNestTables() }, params);
      if (builder.isOne()) {
